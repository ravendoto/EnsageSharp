...
namespace PhantomAssassin
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("> PhantomAssassin# loaded!");
        ...
        public static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (!Game.IsInGame || me.ClassID != ClassID.CDOTA_Unit_Hero_PhantomAssassin)
            {
                return;
            }
            ...
            if (activated)
            {
                ...
                if (target.IsAlive && !target.IsInvul())
                {
                    ...
                    if (target.IsVisible && me.Distance2D(target) <= 1500)
                    {
                        if (Q.CanBeCasted() &&
                            me.Position.Distance2D(target.Position) <= 1200 &&
                            Utils.SleepCheck("Q"))
                        {
                            Q.UseAbility(target);
                            Utils.Sleep(250, "Q");
                        ...
                        if (W.CanBeCasted()                 &&
                            me.Position.Distance2D(target.Position) <= 1000 &&
                            Utils.SleepCheck("W"))
                        {
                            W.UseAbility(target);
                            Utils.Sleep(250, "W");
        ...
        static void Drawing_OnEndScene(EventArgs args)
        {
            ...
            var me = ObjectMgr.LocalHero;
            if (player == null || player.Team == Team.Observer || me.ClassID != ClassID.CDOTA_Unit_Hero_PhantomAssassin)
                return;

            if (activated )
            {
                txt.DrawText(null, "PhantomAssassin#: Comboing!", 4, 150, Color.Green);
            }

            if (!activated)
            {
                txt.DrawText(null, "PhantomAssassin#: go combo  [" + KeyCombo + "] for toggle combo", 4, 150, Color.Aqua);
            }
